...
// loads cipher_alpha and plain test you want to encrypt
    ...
    while((c = getopt_long(argc, argv, short_opt, long_opt, NULL)) != -1) {
        switch(c) {
            case 'h':
                usage(argv[0]);
                return 0;
            case 'm': // if flag -m in input, use following as encrypt method
                enc_method = optarg;
                break;
            case 'i':
                ifile_name = optarg;
...
void substitution_enc(string ifile_name, string ofile_name)
{
    ...
    // cipher alpha is a randomly permuted alphabet
    // plain_text is what we want to encode
